...
fn map(out: &mut impl Write, w: &World, h: usize, cols: usize) {
    ...
    for r in 0..h {
        ...
        for c in 0..mw {
            ...
            match (top, bot) {
                ...
                (Some((e, d)), None) => {
                    let _ = write!(out, "\x1b[0m{}{}", col(RGB.0[e]), UPPER[d]);
                }
                (None, Some((e, d))) => {
                    let _ = write!(out, "\x1b[0m{}{}", col(RGB.0[e]), LOWER[d]);
                }
    ...
    let _ = writeln!(
        out,
        "{DIM}└{}┘  bodies only — nothing to be attracted to until terrain lands at S1{RESET}{CLEAR_EOL}",
        "─".repeat(mw)
    );
}
